...
type Theme = 'light' | 'dark' | 'unicorn';

const THEME_ORDER: Theme[] = ['light', 'dark', 'unicorn'];
...
const toggleThemeCycle = (current: Theme): Theme => {
...
const ThemeToggleIcon: FC<ThemeToggleIconProps> = ({ theme }) => {
  switch (theme) {
    case 'light':
      return (
...
const SetTheme: FC = () => {
  ...
  useEffect(() => {
    const handleRouteChange = (url: string) => {
      ...
      // Optionally reinitialize gradient canvas on route change here
    };
  ...
  // Reinitialize gradient canvas on theme or route change
  useEffect(() => {
    const canvasEle = document.querySelector('#gradient-canvas');
    if (canvasEle) {
      // Rather than embedding all gradient code inline, import and call a utility:
      // e.g., initGradient('#gradient-canvas');
      console.log('Reinitializing gradient for theme:', theme);
    }
  }, [theme, router.asPath]);

  const toggleTheme = () => {
    setTheme(prevTheme => toggleThemeCycle(prevTheme));
  };
    ...
    <>
      <Script id="theme-init" strategy="beforeInteractive">
        {`
          ...
          document.documentElement.dataset.theme = themeLocalStorage || themeSystem;
        `}
      </Script>
      <button
        key="themeToggle"
